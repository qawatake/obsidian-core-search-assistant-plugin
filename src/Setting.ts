			...
			.addDropdown((component) => {
				...
				component
					...
					.onChange((value) => {
						...
						this.plugin.settings.outlineWidth = width as AvailableOutlineWidth;
						this.plugin.saveSettings();
					});
			});
		...
		new Setting(containerEl)
			...
			.addDropdown((component) => {
				component
					...
					.setValue(this.plugin.settings?.autoPreviewMode ?? "cardView")
					.onChange((id) => {
						if (!this.plugin.settings) {
							return;
						}
						if (!AUTO_PREVIEW_MODE_IDS.includes(id as AutoPreviewMode)) {
							return;
						}
						this.plugin.settings.autoPreviewMode = id as AutoPreviewMode;
						this.plugin.saveSettings();
					});
			});
			...
			.addDropdown((component) => {
				...
				component
					...
					.onChange((value) => {
						...
						if (!AVAILABLE_CARD_LAYOUT.includes(value as AvailableCardLayout)) {
							return;
						}
						this.plugin.settings.cardViewLayout = value as AvailableCardLayout;
						this.plugin.saveSettings();
					});
			});
		...
		SEARCH_MODE_HOTKEY_ACTION_IDS.forEach((actionId) => {
			...
			const hotkeySetter = new HotkeySetter(
				...
				if (added) {
					...
					// avoid collision
					const collision = SEARCH_MODE_HOTKEY_ACTION_IDS.some((actionId) => {
						const hotkeys = settings.searchModeHotkeys[actionId];
